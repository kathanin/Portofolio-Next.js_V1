...
const AnimatedBackground = () => {
  // [Perubahan 2: Tipe pada useRef]
  const blobRefs = useRef<(HTMLDivElement | null)[]>([]);

  // [Perubahan 3: Tipe struktur objek]
  ...
  useEffect(() => {
    let currentScroll = 0;
    let requestId: number; // [Perubahan 4]

    const handleScroll = () => {
      const newScroll = window.scrollY; // [Perubahan 5]
      currentScroll = newScroll;

      blobRefs.current.forEach((blob, index) => {
        if (!blob) return; // [Perubahan 6: Validasi keamanan TS]
